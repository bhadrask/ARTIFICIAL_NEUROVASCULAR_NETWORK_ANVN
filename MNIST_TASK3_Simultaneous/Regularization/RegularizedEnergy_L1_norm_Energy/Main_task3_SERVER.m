...
vas_ln_mat =[16,24,32,42,52,64,72,80,96,100:50:500];% [16,32,64,72,80,96,100:25:500];%[10:20:100,150:50:500];%[20,24,28,36,40,44,48,52,56];%[16,32,64,72,80,96,100:25:500]; %[64,125:50:800];
...
wallowable_fraction =0.2:0.2:3;
for i = 1:numel(wallowable_fraction)
%      kmat =[2,3*ones(1,9),4*ones(1,3)];%[3*ones(1,2),4*ones(1,4),8,8,8,8*ones(1,numel(100:25:250)),16*ones(1,numel(275:25:500))];%[3,4*ones(1,2),8*ones(1,3)];%% [3,4*ones(1,2),8*ones(1,3)];%%%[4, 8*ones(1,8), 16*ones(1,6)];
    kmat = [3,4*ones(1,5),8*ones(1,12)]; %[4, 8*ones(1,8), 16*ones(1,6)];
% kmat=[4*ones(1,numel(25:25:50)),8*ones(1,numel(75:25:250)),16*ones(1,numel(275:25:1000))];
    parfor ln_idx =1:size(vas_ln_mat,2)
