...
vas_ln_mat =[64,100,300,500];%[16,24,32,42,52,64,72,80,96,100:25:500];%[16,32,64,100,300,500];%[16,20,24,28,32,36,40,44,48,52,56,64,72,80,96,100:25:500];%[64,125:50:800];%525;%[64,100:25:900];%[64,100:25:500];%,350:25:575];%[64,100,200,300,400,425,450,475,500,525,550] ;%[64,100:25:575];%[64,100:100:400,475:5:525,550:25:700,800:100:2200];%,800:100:2200];%[64,100:100:1000];%[32,50:50:1000];%[16,256,512,768,1024,2048];
...
%         %         title(['Test accuracy vs Total energy consumption neurons ',string(vas_ln_mat(j))]);
        title({'Test accuracy vs Percapita energy consumption',['for hidden neuron number ',num2str(vas_ln_mat(j))]});
...
%         strlabel(1,i) = 'Initial Energy fraction= ' + string(wallowable_fr(i));
%     end
    figure(j);
%     legend(strlabel)
    filename='nonR_E_vs_Acc_space for Hidden neurons= '+ string(vas_ln_mat(j))+'.jpg';
     saveas(gcf,filename);
