function forvarenegy_eta_SERVER(str,vas_ln, k, vas_dim, vas_energy_mat, vas_trials, mlp_train_images, ...
...
load(str,'mlp_W1','mlp_W2','mlp_b1','mlp_b2','tree_Weight');
...
%     mlp_b2 = repmat(rand(mlp_n3,1),1);
    ...
    [tree_Level,tree_Ntot, tree_Weight, tree_Energy, tree_A, tree_Nlevel, tree_leaf_idx,Tree_beforeweight] = re_define_tree(tree_Weight,vas_ln, k, vas_dim, energy_in,rng_set);
    ...
    for i = 1:mlp_epoch
        ...
        eta1 = get_exp_vas_eta(i, mlp_epoch,0.01, 0.001);
        ...
        if rem(i,mlp_save_every) == 0
         ...
         mlp_images=mlp_train_images;
         mlp_labels=mlp_train_labels;
          mlp_train_perm = randperm(mlp_train_size);
        mlp_train_images = mlp_images(:, mlp_train_perm);
        mlp_train_labels = mlp_labels(mlp_train_perm);
         W2=sum(tree_Weight,2);
        diffW=abs(W1-W2);
        for lv=1:max(tree_Level)
            Wdifflevel=diffW(tree_Level==lv);
...
parsave_SERVER(VRMSE,vas_ln, percap_energy_con, mlp_train_success, mlp_test_success, str_dele, wallowable_int,ENDsave_energy,init_energy,tree_W_init,mlp_W1,mlp_W2,mlp_b1,mlp_b2,tree_Weight);
